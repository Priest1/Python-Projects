...
# I am dropping these columns because if they have a body id or boat number then that assumes they did or did not survive
# also the cabin will already be covered by which class they were in
...
# The below function will format the rest of the data so that the SVC algorithm can accept it.
...
# X & y will now be further moved into training & testing sets. The training set will be processed by the SVC algorithm
# to form the model for training. After we have formed the model the program will classify the testing set to determine
